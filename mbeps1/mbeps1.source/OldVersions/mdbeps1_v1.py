...
mfield1.mepois1_init(ffe,in1.ax,affp,wpm,in1.ci,nx)
...
if ((in1.ntp > 0) or (in1.nta > 0) or (in1.ntet > 0)):
...
# deposit current with OpenMP: updates cue
   ...
   mcurd1.wmdjpost1(ppart,cue,kpic,ncl,ihole,in1.qme,zero,in1.ci,
                    tdjpost,nx,in1.mx,ipbc,in1.relativity,False,irc)
...
# deposit ion current with OpenMP: updates cui
   if (in1.movion==1):
      ...
      dtimer(dtime,itime,1)
      tdjpost[0] = tdjpost[0] + float(dtime)
      mcurd1.wmdjpost1(pparti,cui,kipic,ncl,ihole,in1.qmi,zero,in1.ci,
...
# calculate magnetic field in fourier space: updates byze, wb
   mfield1.mbbpois1(cue,byze,ffc,in1.ci,wb,tfield,nx)
...
# updates dcu, amu
   ...
   mdpush1.wmgdjpost1(ppart,exyze,byze,dcu,amu,kpic,in1.omx,in1.qme,
                      qbme,in1.dt,in1.ci,tdcjpost,nx,in1.mx,
                      in1.relativity)
...
# updates dcui, amui
   if (in1.movion==1):
      dtimer(dtime,itime,-1)
      dcui.fill(0.0); amui.fill(0.0)
      dtimer(dtime,itime,1)
      tdcjpost[0] = tdcjpost[0] + float(dtime)
      mdpush1.wmgdjpost1(pparti,exyze,byze,dcui,amui,kipic,in1.omx,
                         in1.qmi,qbmi,in1.dt,in1.ci,tdcjpost,nx,in1.mx,
                         in1.relativity)
...
# updates dcu, amu
   isign = -1
   mfft1.mfft1rn(dcu,isign,mixup,sct,tfft,in1.indx)
...
# calculate transverse electric field: updates cus, wf
   mfield1.mepois1(dcu,cus,ffe,affp,in1.ci,wf,tfield,nx)

# transform transverse electric field to real space: updates cus
   isign = 1
   mfft1.mfft1rn(cus,isign,mixup,sct,tfft,in1.indx)
...
# with OpenMP: updates cue, dcu, amu
      ...
      dtimer(dtime,itime,1)
      tdcjpost[0] = tdcjpost[0] + float(dtime)
      mdpush1.wmgdcjpost1(ppart,exyze,byze,cue,dcu,amu,kpic,in1.omx,
                          in1.qme,qbme,in1.dt,in1.ci,tdcjpost,nx,in1.mx,
                          in1.relativity)
...
# with OpenMP: updates cui, dcui, amui
      if (in1.movion==1):
         dtimer(dtime,itime,-1)
         cui.fill(0.0); dcui.fill(0.0); amui.fill(0.0)
         dtimer(dtime,itime,1)
         tdcjpost[0] = tdcjpost[0] + float(dtime)
         mdpush1.wmgdcjpost1(pparti,exyze,byze,cui,dcui,amui,kipic,
                             in1.omx,in1.qmi,qbmi,in1.dt,in1.ci,
                             tdcjpost,nx,in1.mx,in1.relativity)
# add electron and ion densities: updates cue, dcu, amu
         mfield1.maddcuei1(cue,cui,tfield,nxe)
         mfield1.maddcuei1(dcu,dcui,tfield,nxe)
         mfield1.maddcuei1(amu,amui,tfield,nxe)
...
# calculate vector potential in fourier space: updates vfieldc
         mfield1.mapot1(cue,vfieldc,ffc,in1.ci,ws,tfield,nx)
...
# calculate magnetic field in fourier space: updates vfieldc
         mfield1.mibpois1(cue,vfieldc,ffc,in1.ci,ws,tfield,nx)
...
# updates ppart and wke, and possibly ncl, ihole, irc
   wke[0] = 0.0
   if (in1.mzf==0):
      mbpush1.wmbpush1(ppart,exyze,byze,kpic,ncl,ihole,in1.omx,qbme,
                       in1.dt,in1.dt,in1.ci,wke,tpush,nx,in1.mx,ipbc,
                       in1.relativity,list,irc)
# zero force: updates ppart, wke and possibly ncl, ihole, and irc
   else:
      mbpush1.wmpush1zf(ppart,kpic,ncl,ihole,in1.dt,in1.ci,wke,tpush,
...
# updates pparti and wki, and possibly ncl, ihole, irc
      wki[0] = 0.0
      if (in1.mzf==0):
         mbpush1.wmbpush1(pparti,exyze,byze,kipic,ncl,ihole,in1.omx,
                          qbmi,in1.dt,in1.dt,in1.ci,wki,tpush,nx,in1.mx,
