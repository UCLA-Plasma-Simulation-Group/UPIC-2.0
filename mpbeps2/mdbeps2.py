...
dcu = numpy.empty((in2.ndim,nxe,nypmx[0]),float_type,'F')
# cus = smoothed transverse electric field with guard cells
cus = numpy.empty((in2.ndim,nxe,nypmx[0]),float_type,'F')
# amu = electron momentum flux with guard cells
amu = numpy.empty((mdim,nxe,nypmx[0]),float_type,'F')
# exyze = smoothed total electric field with guard cells
exyze = numpy.empty((in2.ndim,nxe,nypmx[0]),float_type,'F')
...
exyt = numpy.empty((in2.ndim,nye,kxp),complex_type,'F')
...
# dcui = electron/ion acceleration density with guard cells
   dcui = numpy.empty((in2.ndim,nxe,nypmx[0]),float_type,'F')
# amui = electron/ion momentum flux with guard cells
   amui = numpy.empty((mdim,nxe,nypmx[0]),float_type,'F')
...
# find maximum and minimum initial electron density
   mpush2.mpset_pszero2(qe,tinit,in2.mx,in2.my,mx1,myp1)
   mpush2.mppost2(ppart,qe,kpic,noff,in2.qme,tinit,in2.mx,in2.my,mx1,
                  in2.popt)
   ompplib2.wmpaguard2(qe,nyp,tinit,nx,kstrt,nvp)
   if (in2.movion==1):
      mpush2.mpset_pszero2(qi,tinit,in2.mx,in2.my,mx1,myp1)
      mpush2.mppost2(pparti,qi,kipic,noff,in2.qmi,tinit,in2.mx,in2.my,
                     mx1,in2.popt)
      ompplib2.wmpaguard2(qi,nyp,tinit,nx,kstrt,nvp)
      mdpush2.mpfwptminx2(qe,qi,nyp,qbme,qbmi,wpmax,wpmin,nx)
   else:
      mdpush2.mpfwpminx2(qe,nyp,qbme,wpmax,wpmin,nx)
   wtot[0] = wpmax[0]
   wtot[1] = -wpmin[0]
   mppmod2.mpdmax(wtot[:2],tinit)
   wpmax[0] = wtot[0]
   wpmin[0] = -wtot[1]
...
# accelerate convergence: update wpm
   if (wpm[0] <= 10.0):
      wpm[0] = 0.75*wpm[0]
   if (kstrt==1):
      print ("wpm=", wpm[0],file=iuot)
   q2m0[0] = wpm[0]/affp

# initialize darwin electric field
   mpush2.mpset_pvzero2(cus,tinit,in2.mx,in2.my,mx1,myp1)
...
# read in basic restart file for darwin code: updates cus, wpm, q2m0
   sd2.bread_drestart23(cus,wpm,q2m0,iur0)

# calculate form factor: ffe
mfield2.mpepois2_init(ffe,in2.ax,in2.ay,affp,wpm,in2.ci,nx,ny,kstrt)

# initialize longitudinal electric field
mpush2.mpset_pvzero2(fxyze,tinit,in2.mx,in2.my,mx1,myp1)
...
# oldcue = previous current density
   oldcue = numpy.empty((in2.ndim,nxe,nypmx[0]),float_type,'F')
...
# write filename to diagnostic metafile
#     if (sys.version_info.major==3):
#        in2.fjename = fjename.ljust(in2.fjename.dtype.itemsize)
#     else:
#        in2.fjename[:] = fjename
# open file
#     if (in2.njerec==0):
...
# write filename to diagnostic metafile
#        if (sys.version_info.major==3):
#           in2.fjiname = fjiname.ljust(in2.fjiname.dtype.itemsize)
#        else:
#           in2.fjiname[:] = fjiname
# open file
#     if (in2.njirec==0):
...
# write filename to diagnostic metafile
#     if (sys.version_info.major==3):
#        in2.faname = faname.ljust(in2.faname.dtype.itemsize)
#     else:
#        in2.faname[:] = faname
# open file
#     if (in2.narec==0):
...
# write filename to diagnostic metafile
#     if (sys.version_info.major==3):
#        in2.fetname = fetname.ljust(in2.fetname.dtype.itemsize)
#     else:
#        in2.fetname[:] = fetname
# open file
#     if (in2.netrec==0):
...
# write filename to diagnostic metafile
#     if (sys.version_info.major==3):
#        in2.fbname = fbname.ljust(in2.fbname.dtype.itemsize)
#     else:
#        in2.fbname[:] = fbname
# open file
#     if (in2.nbrec==0):
...
if (kstrt==1):
   print ( "program mpfbeps2",file=iuot)
...
# deposit electron current with OpenMP: updates cue
   ...
   mcurd2.wmpdjpost2(ppart,cue,kpic,ncl,ihole,noff,nyp,in2.qme,zero,
...
# deposit ion current with OpenMP: updates cui
   if (in2.movion==1):
      ...
      mcurd2.wmpdjpost2(pparti,cui,kipic,ncl,ihole,noff,nyp,in2.qmi,
...
# updates bxyt, wm
   mfield2.mpbbpois2(cut,bxyt,ffc,in2.ci,wb,tfield,nx,ny,kstrt)
...
# updates exyze
   mfield2.mpaddvrfield2(exyze,cus,fxyze,tfield)

# deposit electron acceleration density and momentum flux with OpenMP:
# updates dcu, amu
   mpush2.mpset_pvzero2(dcu,tdcjpost,in2.mx,in2.my,mx1,myp1)
   mpush2.mpset_pvzero2(amu,tdcjpost,in2.mx,in2.my,mx1,myp1)
   mdpush2.wmpgdjpost2(ppart,exyze,bxyze,dcu,amu,kpic,noff,nyp,in2.qme,
                       qbme,in2.dt,in2.ci,tdcjpost,nx,in2.mx,in2.my,mx1,
                       in2.popt,in2.relativity)
# add guard cells with OpenMP: updates dcu, amu
   ompplib2.wmpnacguard2(dcu,nyp,tguard,nx,kstrt,nvp)
   ompplib2.wmpnacguard2(amu,nyp,tguard,nx,kstrt,nvp)

# deposit ion acceleration density and momentum flux with OpenMP:
# updates dcui, amui
   if (in2.movion==1):
      mpush2.mpset_pvzero2(dcui,tdcjpost,in2.mx,in2.my,mx1,myp1)
      mpush2.mpset_pvzero2(amui,tdcjpost,in2.mx,in2.my,mx1,myp1)
      mdpush2.wmpgdjpost2(pparti,exyze,bxyze,dcui,amui,kipic,noff,nyp,
                          in2.qmi,qbmi,in2.dt,in2.ci,tdcjpost,nx,in2.mx,
                          in2.my,mx1,in2.popt,in2.relativity)
# add guard cells with OpenMP: updates dcui, amui
      ompplib2.wmpnacguard2(dcui,nyp,tguard,nx,kstrt,nvp)
      ompplib2.wmpnacguard2(amui,nyp,tguard,nx,kstrt,nvp)
# add electron and ion momentum flux densities: updates dcu, amu
      mfield2.mpaddcuei2(dcu,dcui,nyp,tfield,nx)
      mfield2.mpaddamui2(amu,amui,nyp,tfield,nx)

# add old scaled electric field with OpenMP: updates dcu
   mdpush2.mpascfguard2(dcu,cus,nyp,q2m0,tdcjpost,nx)
...
# updates dcut, amut, nterf, and ierr, modifies dcu, amu
   isign = -1
   ompplib2.wmpfft2rn(dcu,dcut,noff,nyp,isign,mixup,sct,tfft,tfmov,
                      in2.indx,in2.indy,kstrt,nvp,kyp,ny,nterf,ierr)
...
# updates dcut
   mfield2.mpadcuperp2(dcut,amut,tfield,nx,ny,kstrt)

# calculate transverse electric field with OpenMP: updates exyt, wf
   mfield2.mpepois2(dcut,exyt,ffe,affp,in2.ci,wf,tfield,nx,ny,kstrt)

# transform transverse electric field to real space with OpenMP:
# moves data to non-uniform partition
# updates cus, nterf, and ierr, modifies cur
   isign = 1
   ompplib2.wmpfft2rn(cus,exyt,noff,nyp,isign,mixup,sct,tfft,tfmov,
                      in2.indx,in2.indy,kstrt,nvp,kyp,ny,nterf,ierr)

# copy guard cells with OpenMP: updates cus
   ompplib2.wmpncguard2(cus,nyp,tguard,nx,kstrt,nvp)
...
# inner iteration loop
   for k in range(0,in2.ndc):

# deposit electron current and acceleration density and momentum flux
# with OpenMP: updates cue, dcu, amu
      mpush2.mpset_pvzero2(cue,tdcjpost,in2.mx,in2.my,mx1,myp1)
      mpush2.mpset_pvzero2(dcu,tdcjpost,in2.mx,in2.my,mx1,myp1)
      mpush2.mpset_pvzero2(amu,tdcjpost,in2.mx,in2.my,mx1,myp1)
      mdpush2.wmpgdcjpost2(ppart,exyze,bxyze,cue,dcu,amu,kpic,noff,nyp,
                           in2.qme,qbme,in2.dt,in2.ci,tdcjpost,nx,
...
# with OpenMP: updates cui, dcui, amui
      if (in2.movion==1):
         mpush2.mpset_pvzero2(cui,tdcjpost,in2.mx,in2.my,mx1,myp1)
         mpush2.mpset_pvzero2(dcui,tdcjpost,in2.mx,in2.my,mx1,myp1)
         mpush2.mpset_pvzero2(amui,tdcjpost,in2.mx,in2.my,mx1,myp1)
         mdpush2.wmpgdcjpost2(pparti,exyze,bxyze,cui,dcui,amui,kipic,
                              noff,nyp,in2.qmi,qbmi,in2.dt,in2.ci,
                              tdcjpost,nx,in2.mx,in2.my,mx1,in2.popt,
                              in2.relativity)
# add guard cells for current, acceleration density, and momentum flux
# with OpenMP: updates cui, dcui, amui
         ompplib2.wmpnacguard2(cui,nyp,tguard,nx,kstrt,nvp)
...
# calculate vector potential in fourier space: updates vfieldc
         mfield2.mpapot2(cut,vfieldc,ffc,in2.ci,ws,tfield,nx,ny,kstrt)
...
# calculate transverse efield in fourier space: updates vfieldc
         mfield2.mpetfield2(dcut,vfieldc,ffe,affp,in2.ci,ws,tfield,nx,
                            ny,kstrt)
...
# calculate magnetic field in fourier space: updates vfieldc
         mfield2.mpibpois2(cut,vfieldc,ffc,in2.ci,ws,tfield,nx,ny,kstrt)
...
# calculate electron fluid moments
         if ((in2.ndfm==1) or (in2.ndfm==3)):
            ...
            mdiag2.wmgbprofx2(ppart,exyze,bxyze,fmse,kpic,noff,nyp,qbme,
...
# calculate ion fluid moments
         if (in2.movion==1):
            if ((in2.ndfm==2) or (in2.ndfm==3)):
               ...
               mdiag2.wmgbprofx2(pparti,exyze,bxyze,fmsi,kipic,noff,nyp,
                                 qbmi,in2.dt,in2.ci,tdiag,in2.npro,nx,
...
# updates ppart and wke, and possibly ncl, ihole, irc
   wke[0] = 0.0
   mbpush2.wmpbpush2(ppart,exyze,bxyze,kpic,ncl,ihole,noff,nyp,qbme,
                     in2.dt,in2.dt,in2.ci,wke,tpush,nx,ny,in2.mx,in2.my,
...
# updates pparti and wki, and possibly ncl, ihole, irc
      wki[0] = 0.0
      mbpush2.wmpbpush2(pparti,exyze,bxyze,kipic,ncl,ihole,noff,nyp,
                        qbmi,in2.dt,in2.dt,in2.ci,wki,tpush,nx,ny,
                        in2.mx,in2.my,mx1,ipbc,in2.popt,in2.relativity,
                        plist,irc)
...
# write out basic restart file for darwin code
         sd2.bwrite_drestart23(cus,wpm,q2m0,iur)
...
if (kstrt==1):
   print (file=iuot)
   print ("ntime,relativity,ndc=",ntime,",",in2.relativity,",",in2.ndc,
          file=iuot)
   if (in2.treverse==1):
...
if (in2.ntv > 0):
   if ((in2.nvft==1) or (in2.nvft==3)):
      ...
      pgraf1.pdisplayfvt1(fvtm,' ELECT',ts,in2.dt*float(in2.ntv),kstrt,
                          itv,ierr)
# ions
      if (in2.movion==1):
         pgraf1.pdisplayfvt1(fvtmi,' ION',ts,in2.dt*float(in2.ntv),
                             kstrt,itv,ierr)
