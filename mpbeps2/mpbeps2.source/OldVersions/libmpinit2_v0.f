...
! j is in the range 1:npx, and k is in the range 1:npy.  The spatial
! location x, y, of the particle corresponding to the index j, k is then
! found by integrating the density profile.  particles are not
! necessarily in the correct processor, and may need to be moved to
! another processor.
! part(1,n) = position x of particle n in partition
! part(2,n) = position y of particle n in partition
! npp = number of particles in partition, updated in this procedure
...
!-----------------------------------------------------------------------
     ...
     &,argy3,npx,npy,nx,ny,idimp,npmax,kstrt,nvp,ipbc,ierr)
! for 2d code, this subroutine calculates initial particle co-ordinates
...
! with spatial decomposition
      ...
      external fnx, fny
! local data
      integer nps, npt, nppv, nppvs, ks, kc, jc, i, j, k, n, nn, noff
      integer imax
      real edgelx, edgely, anx, any, bnx, bny, xt0, yt0, x0, y0
      real xn, yn, eps, big, f, fp
      double precision xt, yt, dnpxy, dnoff, dnpx, dng
      integer ierr1, iwork1
      double precision sum1, work1
      dimension ierr1(1), iwork1(1), sum1(1), work1(1)
      ierr = 0
...
! guess next value for yt
      ...
      i = 0
   10 f = bny*fny(yt,argy1,argy2,argy3,1) - dble(yn)
...
! guess next value for xt
      ...
      i = 0
   30 f = bnx*fnx(xt,argx1,argx2,argx3,1) - dble(xn)
...
! guess next value for yt
         ...
         yt = max(edgely,min(yt,any))
         i = 0
   70    f = bny*fny(yt,argy1,argy2,argy3,1) - dble(yn)
! find improved value for yt
         if (abs(f).ge.eps) then
            fp = bny*fny(yt,argy1,argy2,argy3,0)
! newton's method
