...
! 2D MPI/OpenMP/Vector PIC Code:
! WPFFT2RINIT calculates tables needed by 2d FFTs
! WPPFFT2RVM wrapper function for scalar 2d real/complex FFT
! WPPFFT2RVM2 wrapper function for 2 component vector 2d real/complex
!             FFT
! WPPFFT2RVM3 wrapper function for 3 component vector 2d real/complex
!             FFT
! WPPFFT2RVMN wrapper function for n component vector 2d real/complex
!             FFT
! PPFFT2RVMXX performs x part of scalar 2d real/complex FFT
! PPFFT2RVMXY performs y part of scalar 2d real/complex FFT
! PPFFT2RVM2XX performs x part of 2 component vector 2d real/complex FFT
! PPFFT2RVM2XY performs y part of 2 component vector 2d real/complex FFT
! PPFFT2RVM3XX performs x part of 3 component vector 2d real/complex FFT
! PPFFT2RVM3XY performs y part of 3 component vector 2d real/complex FFT
! PPFFT2RVMNXX performs x part of n component vector 2d real/complex FFT
! PPFFT2RVMNXY performs y part of n component vector 2d real/complex FFT
...
! inverse fourier transform
      if (isign.lt.0) then
! perform x fft
         call PPFFT2RVMXX(f,isign,mixup,sct,indx,indy,kstrt,kypi,kypp,  &
...
! transpose f array to g
     ...
     &kypd)
         call PWTIMERA(1,ttp,dtime)
! perform y fft
         call PPFFT2RVMXY(g,isign,mixup,sct,indx,indy,kstrt,kxpi,kxpp,  &
     &nyv,kxp,nxhyd,nxyhd)
...
!-----------------------------------------------------------------------
      subroutine WPPFFT2RVM2(f,g,bs,br,isign,ntpose,mixup,sct,ttp,indx,  &
     &indy,kstrt,nvp,nxvh,nyv,kxp,kyp,kypd,nxhyd,nxyhd)
...
! inverse fourier transform
      if (isign.lt.0) then
! perform x fft
         call PPFFT2RVM2XX(f,isign,mixup,sct,indx,indy,kstrt,kypi,kypp, &
     &nxvh,kypd,nxhyd,nxyhd)
! transpose f array to g
         call PWTIMERA(-1,ttp,dtime)
     ...
     &,kypd)
         call PWTIMERA(1,ttp,dtime)
! perform y fft
         call PPFFT2RVM2XY(g,isign,mixup,sct,indx,indy,kstrt,kxpi,kxpp, &
     &nyv,kxp,nxhyd,nxyhd)
! transpose g array to f
         if (ntpose.eq.0) then
            call PWTIMERA(-1,tf,dtime)
...
!-----------------------------------------------------------------------
      subroutine WPPFFT2RVM3(f,g,bs,br,isign,ntpose,mixup,sct,ttp,indx, &
     &indy,kstrt,nvp,nxvh,nyv,kxp,kyp,kypd,nxhyd,nxyhd)
...
! inverse fourier transform
      if (isign.lt.0) then
! perform x fft
         call PPFFT2RVM3XX(f,isign,mixup,sct,indx,indy,kstrt,kypi,kypp, &
     &nxvh,kypd,nxhyd,nxyhd)
! transpose f array to g
         call PWTIMERA(-1,ttp,dtime)
     ...
     &,kypd)
         call PWTIMERA(1,ttp,dtime)
! perform y fft
         call PPFFT2RVM3XY(g,isign,mixup,sct,indx,indy,kstrt,kxpi,kxpp, &
     &nyv,kxp,nxhyd,nxyhd)
! transpose g array to f
         if (ntpose.eq.0) then
            call PWTIMERA(-1,tf,dtime)
...
!-----------------------------------------------------------------------
      subroutine WPPFFT2RVMN(f,g,bs,br,ss,isign,ntpose,mixup,sct,ttp,   &
...
! inverse fourier transform
      if (isign.lt.0) then
! perform x fft
         call PPFFT2RVMNXX(f,ss,isign,mixup,sct,indx,indy,kstrt,kypi,   &
...
! transpose f array to g
     ...
     &kxp,kypd)
         call PWTIMERA(1,ttp,dtime)
! perform y fft
         call PPFFT2RVMNXY(g,isign,mixup,sct,indx,indy,kstrt,kxpi,kxpp, &
...
!-----------------------------------------------------------------------
      subroutine PPFFT2RVMXX(f,isign,mixup,sct,indx,indy,kstrt,kypi,kypp&
     &,nxvh,kypd,nxhyd,nxyhd)
...
! local data
      ...
      integer i, m, ns, ns2, km, kmr, k1, k2, j1, nrxb
      real ani
      complex s, t, t1
      indx1 = indx - 1
      indx1y = max0(indx1,indy)
...
! inverse fourier transform
      ...
      nrx = nxy/nxh
!$OMP PARALLEL DO                                                       &
!$OMP& PRIVATE(i,j,k,m,ns,ns2,km,kmr,k1,k2,j1,s,t,t1)
      do 60 i = kypi, kypt
...
!-----------------------------------------------------------------------
      subroutine PPFFT2RVMXY(g,isign,mixup,sct,indx,indy,kstrt,kxpi,kxpp&
     &,nyv,kxp,nxhyd,nxyhd)
...
! local data
      ...
      integer i, m, ns, ns2, km, kmr, k1, k2, j1, nryb
      complex s, t
      indx1 = indx - 1
      indx1y = max0(indx1,indy)
      nx = 2**indx
...
! inverse fourier transform
      ...
      nry = nxy/ny
!$OMP PARALLEL DO                                                       &
!$OMP& PRIVATE(i,j,k,m,ns,ns2,km,kmr,k1,k2,j1,s,t)
      do 50 i = kxpi, kxpt
...
! inverse fourier transform
      ...
      nrx = nxy/nxh
!$OMP PARALLEL DO                                                       &
!$OMP& PRIVATE(i,j,k,m,ns,ns2,km,kmr,k1,k2,j1,at1,s,t,t1,t2)
      do 90 i = kypi, kypt
...
! then transform in x
      ...
      t1 = s*f(1,j+k2,i)
      t2 = s*f(2,j+k2,i)
      f(1,j+k2,i) = f(1,j+k1,i) - t1
      f(2,j+k2,i) = f(2,j+k1,i) - t2
      f(1,j+k1,i) = f(1,j+k1,i) + t1
      f(2,j+k1,i) = f(2,j+k1,i) + t2
   30 continue
   40 continue
   50 continue
! unscramble coefficients and normalize
...
!-----------------------------------------------------------------------
      subroutine PPFFT2RVM2XY(g,isign,mixup,sct,indx,indy,kstrt,kxpi,   &
...
! inverse fourier transform
      ...
      nry = nxy/ny
!$OMP PARALLEL DO                                                       &
!$OMP& PRIVATE(i,j,k,m,ns,ns2,km,kmr,k1,k2,j1,s,t1,t2)
      do 50 i = kxpi, kxpt
! bit-reverse array elements in y
      do 10 k = 1, ny
      k1 = (mixup(k) - 1)/nryb + 1
...
! then transform in y
      ...
      t1 = s*g(1,j+k2,i)
      t2 = s*g(2,j+k2,i)
      g(1,j+k2,i) = g(1,j+k1,i) - t1
      g(2,j+k2,i) = g(2,j+k1,i) - t2
      g(1,j+k1,i) = g(1,j+k1,i) + t1
      g(2,j+k1,i) = g(2,j+k1,i) + t2
   20 continue
   30 continue
   40 continue
   50 continue
...
!-----------------------------------------------------------------------
      subroutine PPFFT2RVM3XX(f,isign,mixup,sct,indx,indy,kstrt,kypi,   &
     &kypp,nxvh,kypd,nxhyd,nxyhd)
...
! inverse fourier transform
      ...
      nrx = nxy/nxh
!$OMP PARALLEL DO                                                       &
!$OMP& PRIVATE(i,j,k,m,ns,ns2,km,kmr,k1,k2,j1,at1,at2,s,t,t1,t2,t3)
      do 90 i = kypi, kypt
! swap complex components
      do 10 j = 1, nxh
      at1 = real(f(3,j,i))
...
! then transform in x
      ...
      f(1,j+k2,i) = f(1,j+k1,i) - t1
      f(2,j+k2,i) = f(2,j+k1,i) - t2
      f(3,j+k2,i) = f(3,j+k1,i) - t3
      f(1,j+k1,i) = f(1,j+k1,i) + t1
      f(2,j+k1,i) = f(2,j+k1,i) + t2
      f(3,j+k1,i) = f(3,j+k1,i) + t3
   30 continue
   40 continue
   50 continue
! unscramble coefficients and normalize
...
!-----------------------------------------------------------------------
      subroutine PPFFT2RVM3XY(g,isign,mixup,sct,indx,indy,kstrt,kxpi,   &
     &kxpp,nyv,kxp,nxhyd,nxyhd)
...
! inverse fourier transform
      ...
      nry = nxy/ny
!$OMP PARALLEL DO                                                       &
!$OMP& PRIVATE(i,j,k,m,ns,ns2,km,kmr,k1,k2,j1,s,t1,t2,t3)
      do 50 i = kxpi, kxpt
! bit-reverse array elements in y
      do 10 k = 1, ny
      k1 = (mixup(k) - 1)/nryb + 1
...
! then transform in y
      ...
      g(1,j+k2,i) = g(1,j+k1,i) - t1
      g(2,j+k2,i) = g(2,j+k1,i) - t2
      g(3,j+k2,i) = g(3,j+k1,i) - t3
      g(1,j+k1,i) = g(1,j+k1,i) + t1
      g(2,j+k1,i) = g(2,j+k1,i) + t2
      g(3,j+k1,i) = g(3,j+k1,i) + t3
   20 continue
   30 continue
   40 continue
   50 continue
...
!-----------------------------------------------------------------------
      subroutine PPFFT2RVMNXX(f,ss,isign,mixup,sct,indx,indy,kstrt,kypi,&
     &kypp,nxvh,kypd,ndim,nxhyd,nxyhd)
! this subroutine performs the x part of N two dimensional real to
! complex fast fourier transforms and their inverses, for a subset of y,
! using complex arithmetic, where N = ndim, with Vector/OpenMP,
! for data which is distributed in blocks
! for isign = (-1,1), input: all, output: f
...
! local data
      integer indx1, indx1y, nx, nxh, nxhh, nxh2, ny
      integer nxy, nxhy, kypt, j, k, nrx
      integer i, m, ns, ns2, km, kmr, k1, k2, j1, jj, nrxb
      real ani
      complex s, t, t1
      indx1 = indx - 1
      indx1y = max0(indx1,indy)
...
!$OMP PARALLEL DO                                                       &
!$OMP& PRIVATE(i,j,k,m,ns,ns2,km,kmr,k1,k2,jj,j1,s,t,t1)
      do 100 i = kypi, kypt
! bit-reverse array elements in x
      do 20 j = 1, nxh
      j1 = (mixup(j) - 1)/nrxb + 1
...
! then transform in x
      ...
      do 30 jj = 1, ndim
      t1 = s*f(jj,j+k2,i)
      f(jj,j+k2,i) = f(jj,j+k1,i) - t1
      f(jj,j+k1,i) = f(jj,j+k1,i) + t1
   30 continue
   40 continue
   50 continue
   60 continue
...
!-----------------------------------------------------------------------
      subroutine PPFFT2RVMNXY(g,isign,mixup,sct,indx,indy,kstrt,kxpi,   &
...
! local data
      integer indx1, indx1y, nx, nxh, ny, nyh, ny2
      integer nxy, nxhy, ks, kxpt, j, k, nry
      integer i, m, ns, ns2, km, kmr, k1, k2, j1, jj, nryb
      complex s, t1
      indx1 = indx - 1
      indx1y = max0(indx1,indy)
      nx = 2**indx
...
! inverse fourier transform
      ...
      nry = nxy/ny
!$OMP PARALLEL DO                                                       &
!$OMP& PRIVATE(i,j,k,m,ns,ns2,km,kmr,k1,k2,jj,j1,s,t1)
      do 70 i = kxpi, kxpt
! bit-reverse array elements in y
      do 20 k = 1, ny
      k1 = (mixup(k) - 1)/nryb + 1
...
! then transform in y
      ...
      do 30 jj = 1, ndim
      t1 = s*g(jj,j+k2,i)
      g(jj,j+k2,i) = g(jj,j+k1,i) - t1
      g(jj,j+k1,i) = g(jj,j+k1,i) + t1
   30 continue
   40 continue
   50 continue
   60 continue
