...
! ndim = number of velocity coordinates = 3
      integer, parameter :: ndim = 3, mdim = 4
...
! cue = current density with guard cells
      real, dimension(:,:,:), allocatable :: cue, amue
...
! bxyze = magnetic field with guard cells
      real, dimension(:,:,:), allocatable :: fxyze, axyze, bxyze, exyze
...
! cut = current density in fourier space
      real, dimension(:,:,:), allocatable :: cut, dcut, amut
...
! bxyt = magnetic field in fourier space
      real, dimension(:,:,:), allocatable :: fxyt, axyt, bxyt, exyt
! ffd, ffe = form factor array for poisson solvers
      complex, dimension(:,:), allocatable :: ffd, ffe
...
! dirichlet boundary conditions with method 1
      ...
      real, dimension(:,:,:), allocatable :: cue2, amue2
      real, dimension(:,:,:), allocatable :: fxye2, axye2, bxye2, exye2
      complex, dimension(:,:), allocatable :: qt2, pott2
      complex, dimension(:,:,:), allocatable :: cut2, dcut2, amut2
      complex, dimension(:,:,:), allocatable :: fxyt2, axyt2, bxyt2, exyt2
...
! allocate and initialize data for standard code
      ...
      allocate(cue(ndim,nxe,nypmx),amue(mdim,nxe,nypmx))
      allocate(fxyze(ndim,nxe,nypmx),axyze(ndim,nxe,nypmx))
      allocate(bxyze(ndim,nxe,nypmx),exyze(ndim,nxe,nypmx))
      allocate(qt(nye,kxp2+1),pott(nye,kxp2+1))
      allocate(cut(ndim,nye,kxp2+1),dcut(ndim,nye,kxp2+1))
      allocate(amut(mdim,nye,kxp2+1))
      allocate(fxyt(ndim,nye,kxp2+1),axyt(ndim,nye,kxp2+1))
      allocate(bxyt(ndim,nye,kxp2+1),exyt(ndim,nye,kxp2+1))
      allocate(ffd(ny,kxp2),ffe(ny,kxp2))
      allocate(mixup(nxhy),sctd(nxy))
!
! dirichlet boundary conditions with method 1
      allocate(qe2(nx2e,kyp2),pot2(nx2e,kyp2))
      allocate(cue2(ndim,nx2e,kyp2),amue2(mdim,nx2e,kyp2))
      allocate(fxye2(ndim,nx2e,kyp2),axye2(ndim,nx2e,kyp2))
      allocate(bxye2(ndim,nx2e,kyp2),exye2(ndim,nx2e,kyp2))
      allocate(qt2(ny2,kxp2),pott2(ny2,kxp2))
      allocate(cut2(ndim,ny2,kxp2),dcut2(ndim,ny2,kxp2))
      allocate(amut2(mdim,ny2,kxp2))
      allocate(fxyt2(ndim,ny2,kxp2),axyt2(ndim,ny2,kxp2))
      allocate(bxyt2(ndim,ny2,kxp2),exyt2(ndim,ny2,kxp2))
...
! calculate form factor: ffd
      ...
      call mpepoisd2_init(ffe,ax,ay,affp,wpm,ci,nx,ny,kstrt)
!
! dirichlet boundary conditions with method 1
      call mpfft2_init(mixup2,sct2,indx1,indy1)
!     call mppois2_init(ffd,ax,ay,affp,nx2,ny2,kstrt)
!     call mpepois2_init(ffe,ax,ay,affp,wpm,ci,nx2,ny2,kstrt)
!
...
!
! deposit point momentum flux (0,-1,0.5,-0.5) at global location
! x = nx/4, y = ny/4
      amue = 0.0
      k = ny/4 - noff
! deposit to correct local node
...
!
! moves vector grid amue from non-uniform to uniform partition
      isign = -1
      call mpfnmove2(amue,noff,nyp,isign,tfmov,kyp,ny,kstrt,nvp,nterf,  &
     &irc)
      if (irc /= 0) then
         call PPEXIT()
         stop
...
!
! Fourier Transform current derivatives from Real Space with Method 1:
      if (ksolve==1) then
! create image current derivatives for dirichlet boundary conditions:
! updates amue2
         call mpdblsin2m(amue,amue2,tfmov,nx,ny,kstrt,nvp,kyp)
!
! transform acceleration density and momentum flux to fourier space
! updates amut2, modifies amu3e2
         isign = -1
         call mpfft2rn(amue2,amut2,isign,mixup2,sct2,tfft,indx1,indy1,  &
     &kstrt,nvp,kyp2)
!
! Fourier Transform current derivatives from Real Space with Method 2:
      else if (ksolve==2) then
! transform acceleration density and momentum flux with fast sine/cosine
! transforms: updates amut, modifies amue
         isign = -1
         call mpfsct2rm(amue,amut,isign,mixup,sctd,tfft,indx,indy,kstrt,&
     &nvp,kxp2,kyp)
      endif
!
! zero out acceleration density
      dcut = 0; dcut2 = cmplx(0.0,0.0)
!
! Take transverse part of time derivative of current with Method 1:
      if (ksolve==1) then
! using fourier series: updates dcut2
         call mpadcuperp2(dcut2,amut2,tfield,nx2,ny2,kstrt)
!
! Take transverse part of time derivative of current with Method 2:
      else if (ksolve==2) then
! using sine/cosine transforms: updates dcut
         call mpadcuperpd2(dcut,amut,tfield,nx,ny,kstrt)
      endif
!
! Solve Transverse Electric Field with Method 1:
      if (ksolve==1) then
! calculate transverse electric field in fourier space:
! updates exyt2, wf
         call mpetfield2(dcut2,exyt2,ffe,affp,ci,wf,tfield,nx2,ny2,kstrt)
!
! Solve Transverse Electric Field with Method 2:
      else if (ksolve==2) then
! calculate transverse electric field with fast sine-cosine transforms:
! updates exyt, wf
         call mpetfieldd2(dcut,exyt,ffe,affp,ci,wf,tfield,nx,ny,kstrt)
      endif
!
! Fourier Transform to Real Space with Method 1:
      if (ksolve==1) then
! transform transverse electric field to real space:
! updates exye2, modifies exyt2
         isign = 1
         call mpfft2rn(exye2,exyt2,isign,mixup2,sct2,tfft,indx1,indy1,  &
     &kstrt,nvp,kyp2)
!
! copy solution to primary array exyze: updates exyze, exye2
         call mphafdbl2c(exyze,exye2,tfmov,nx,ny,kstrt,nvp,kyp)
!
! Fourier Transform to Real Space with Method 2:
      else if (ksolve==2) then
! transform transverse electric field to real space with fast
! sine/cosine transforms: updates exyze, modifies exyt
         isign = 1
         call mpfcst2rn(exyze,exyt,isign,mixup,sctd,tfft,indx,indy,kstrt&
     &,nvp,kxp2,kyp)
      endif
!
! moves vector grid exyze from uniform to non-uniform partition
      isign = 1
      call mpfnmove2(exyze,noff,nyp,isign,tfmov,kyp,ny,kstrt,nvp,nterf, &
     &irc)
!
! display transverse electric field
      call mpnlcguard2(exyze,nyp,tfield,kstrt,nvp)
      call pdvector2(exyze,nyp,nvp,'ET FIELD',0,999,0,idt,1,nx,ny,irc)
!
      if (kstrt==1) then
         write (*,*)
